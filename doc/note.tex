...
However, many ecological and evolutionary dynamics are intrinsically stochastic.
...
\end{align}
Writing $\hat{\rho}$ and $\hat{\kappa}_D/\hat{\kappa}_C$ to denote niche overlap and fitness difference in the absence of stochasticity, we have:
\begin{align}
\rho &= \hat{\rho}\exp\left(- \frac{(\sigma_R - \sigma_T)^2 + (\sigma_S - \sigma_P)^2}{4} \right)\\
\frac{\kappa_D}{\kappa_C} &= \frac{\hat{\kappa}_D}{\hat{\kappa}_C} \exp\left(\frac{(\sigma_R - \sigma_T)^2 - (\sigma_S - \sigma_P)^2}{4} \right).
\end{align}
Therefore, stochasticity always reduces niche overlap, thereby increasing niche difference and stabilizing competition.
In contrast, it can have opposing effects on 
