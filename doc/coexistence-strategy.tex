...
We show that SCT recovers the classic classification of two-strategy games and also allows us to compare mechanisms for the evolution of cooperation within a shared niche-fitness difference space.
...
So far, MCT has been primarily utilized for understanding species coexistence, but recent work showed that an analogous framework can be derived for other biological systems, especially for studying pathogen strain competition, allowing one to directly quantify niche and fitness differences of competing entities, be they species or strains \citep{sieben2022quantifying,park2024predicting,park2026unifying}.
...
In contrast, the multiplayer public-goods application considers an explicit $N$-player game with nonlinear public-goods benefits.
...
Strategic coexistence theory decomposes two boundary invasion growth rates from pairwise replicator dynamics into stabilizing and equalizing mechanisms.
...
We note that niche and fitness differences can be equivalently written in terms of the invasion growth rates at the monomorphic boundary conditions:
\begin{align}
...
This parameterization is particularly useful because it allows us to compute niche and fitness differences even when the selection gradient governing replicator dynamics does not uniquely separate into two payoff values.
Since this decomposition depends only on the two boundary invasion rates, it does not uniquely identify the biological mechanisms that generate niche and fitness differences.
...
\caption{
\textbf{Coexistence-theoretic decomposition of classic two-strategy games.}
...
For simplicity, we focus on the effect of number of groups $m$ for group selection; increasing the maximum group size $n$ has the opposite effect (Supplementary Text S4).
...
However, many ecological and evolutionary processes are intrinsically stochastic \citep{traulsen2004stochastic,huang2015stochastic,hilbe2018evolution,zheng2018environmental,su2019evolutionary}.
...
Interestingly, stochasticity can also generate bistable coexistence and multiple internal equilibria \citep{wang2025evolution}.
...
More complicated interior dynamics can arise for different payoff matrices, further highlighting the limitations of predicting coexistence from boundary invasibility conditions (Figure S1).

\section*{Evolution of cooperation in multiplayer, nonlinear public goods games}

Previous case studies focused on evolutionary games based on pairwise interactions, but real-world behaviors, especially cooperation, often involve interactions among multiple players \citep{hamburger1973n,fox1978public,motro1991co,archetti2012game,alvarez2021evolutionary}.
In particular, cooperators can contribute to shared public goods, from which other individuals can benefit \citep{szabo2002phase,hauser2019social,hauert2024frequency,sheng2026cooperation}.
...
\end{equation}
This generalizes the result of \cite{gore2009snowdrift}, showing that the effect of concave nonlinearity on coexistence is robust to multiplayer extension (see Supplementary Text S6 and Figure S2 for detailed analyses).
...
Third, we considered the effect of environmental stochasticity on strategy competition, showing that stochasticity cannot destabilize competition at the boundary \citep{wang2025evolution}.
Finally, a case study of multiplayer, public-goods games showed that nonlinear public benefit can either stabilize or destabilize competition \citep{motro1991co,archetti2012game}.
...
While SCT can be still applied to multi-player interactions, conclusions based on 2-player games may not be robust to multi-player extensions, especially when nonlinearity is introduced.
...
Finally, we have focused on simple examples to illustrate the utility of SCT, but many games involve additional complexities, including population structure \citep{nowak2010evolutionary}, spatial structure \citep{szabo2002phase,hauert2004spatial}, and explicit ecological feedback \citep{weitz2016oscillating,tarnita2025reconciling}.
...
For example, even when the abundances of both competitors are declining, the frequency of the superior competitor can still increase.
Thus, invasibility in frequency and abundance spaces is fundamentally different. 
Moreover, the classic MCT is derived from a simple, Lotka-Volterra model, which permits at most one internal equilibrium and allows us to predict coexistence from mutual invasion.
However, SCT relies on replicator dynamics, which can permit multiple internal equilibria, meaning that criterion for mutual invasion at the boundary and coexistence can differ.
...
\begin{align}
-\frac{r_{i|j}+r_{j|i}}{2} < \frac{r_{j|i}-r_{i|j}}{2} < \frac{r_{i|j} + r_{j|i}}{2}.
\end{align}
Exponentiating all sides preserves the inequality and keep them positive:
\begin{align}
\exp\left(-\frac{r_{i|j}+r_{j|i}}{2}\right) < \exp\left(\frac{r_{j|i}-r_{i|j}}{2}\right) < \exp\left(\frac{r_{i|j} + r_{j|i}}{2}\right).
\end{align}
Thus, by defining niche overlap $\rho$ and fitness difference $\kappa_j/\kappa_i$ as follows,
\begin{align}
\rho &= \exp\left(-\frac{r_{i|j}+r_{j|i}}{2}\right),\\
\frac{\kappa_j}{\kappa_i} &= \exp\left(\frac{r_{j|i}-r_{i|j}}{2}\right),
\end{align}
we obtain the following inequality that defines conditions for mutual invasion:
\begin{equation}
...
\end{align}
where $\pi'_i(\mathbf{x}) = s \pi_i(\mathbf{x})$ is the scaled payoff.
...
\subsection*{S5 Applications to evolutionary games with environmental stochasticity}
...
\begin{align}
1 -\rho &= 1 - \exp\left(\frac{G(1)-G(0)}{2}\right)\\
...
\subsection*{S6 Microbial cooperation and public goods}
...
We note that this is a phenomenological approximation of a public-goods game as it does not explicitly consider interactions between multiple players \citep{archetti2012game}.
...
&= f(1-f)(\epsilon - c)
\end{align}
For this linear growth model, the payoff difference is fixed to $\epsilon-c$ independent of $f$ (Figure S2A).
...
\end{align}
Under this nonlinear assumption, the payoff difference decreases with the cooperator fraction $f$, which can promote the coexistence of cooperators and defectors (Figure S2A): cooperators have a competitive advantage when rare, whereas defectors have a competitive advantage when cooperators are abundant.
...
Instead, niche difference is generated by the concave relationship between glucose availability and microbial growth, which increases from 0 to $1-\exp(-1/2)$ as $\alpha$ goes from 1 to 0  (Figure S2B,C).
In contrast, as $\alpha \to 0^+$, the fitness difference approaches 
\begin{equation}
\log(\kappa_D/\kappa_C) \to c-\frac{1}{2},
\end{equation}
which no longer depends on the private capture efficiency $\epsilon$ (Figure S2C). 
Thus, strong concavity not only generates stabilizing niche differences, but also acts as an equalizing mechanism (Figure S2C).
...
\begin{figure}[!th]
\includegraphics[width=\textwidth]{../diagram_stoch/diagram_stoch_v2.pdf}
\caption{
\textbf{Effects of environmental stochasticity on strategic competition under alternative payoff matrix.}
(A) Assumed deterministic payoff structures across four scenarios.
Environmental stochasticity is assumed to be proportional to these payoffs, scaled by a term $k$.
(B) Arrows indicate the direction of changes in dynamical regime as the amount of stochasticity $k$ increases.
Here, niche difference is represented as $-\log(\rho)$ for illustrative purposes.
(C) Bifurcation diagrams under each scenario as a function of $k$. 
Solid and dashed lines represent stable and unstable equilibria, respectively.
}
\end{figure}
