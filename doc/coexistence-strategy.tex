...
\section*{Evolution of cooperation in multi-player, nonliear public goodsgames}
...
Thus, we can compute niche overlap $\rho$ and fitness difference $\kappa_C/\kappa_D$ using standard SCT, which in turn can be simplified in terms of payoff differences at the boundary:
\begin{align}
...
In this case, cooperators competitively exclude defectors when the marginal benefit $\beta(1) - \beta(0)$ is greater than cost $c$, and defectors competitively exclude cooperators otherwise.
Stable coexistence becomes impossible.
Second, when the benefit of public good $\beta(i)$ is a concave, increasing function of $i$ (i.e., $d\beta/di > 0$ and $d^2\beta/di^2 < 0$), we have $\beta(1)-\beta(0) >  \beta(N) - \beta(N-1)$.
This means that nonliearity can generate niche difference and allow coexistence, provided that the cost $c$ lies between the inequality.
This generalizes the result of \cite{gore2009snowdrift}, showing that the effect of concave nonlinearity on coexistence is robust to multiplayer extension (see Supplementary Text S1 and Figure S1 for detailed analyses).
Third, when the benefit of public good $\beta(i)$ is a convex, increasing function of $i$ (i.e., $d\beta/di > 0$ and $d^2\beta/di^2 > 0$), we have $\beta(1)-\beta(0) < \beta(N) - \beta(N-1)$.
In this case, nonliearity can allow bistability of two boundary conditions and generate priority effect.

We note that this multi-player game can also permit 
...
In particular, the mutual invasion condition at the monomorphic boundary can be written as:
\begin{align}
0 &< G(0)\\
0 &< -G(1)
\end{align}
