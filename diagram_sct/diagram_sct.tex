...
\rho < \frac{\kappa_j}{\kappa_i} < \frac{1}{\rho}
  $\\
  \vspace{0.8em}
  Stabilizing niche differences must be greater than average fitness differences
};
...
\begin{scope}[shift={(14,-4.5)}, x=3cm, y=2.52cm]
  ...
  % region labels
  \node[font=\footnotesize, align=center, text width=1.7cm,fill=white] at (-0.04,0.43)
    {$j$ excludes $i$};

  \node[font=\footnotesize, align=center, text width=1.7cm,fill=white] at (0.35,0.00)
    {Coexist-\\ence};
